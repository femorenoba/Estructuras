...
import my.packages.busqueda.Searcher;
...
public class Catalogo { 
	...
	public DynamicArray<Tomo> buscarPorAutor(String nombre){
		
		DynamicArray<Tomo> res = new DynamicArray();
		...
		DynamicArray<String> resultado = buscador.buscarTomo(nombre, 1);
		
		for(int i = 0; i < resultado.getSize(); ++i)
		{
			res.pushBack(this.buscarTomo(resultado.get(i)));
		}
	...
	private void index() {
		int i = 0;
		for(; i < this.getTomos().getSize(); ++i)
		{
			Document temp = buscador.crearTomoIndex(this.getTomos().get(i));
			try {
				buscador.indexTomo(temp);
			} catch (Exception e) {
				System.out.println("Error al momento de indexar a" + " " + this.getTomos().get(i).getNombre());
			}
		}
		this.existeIndex = true;
	}
	public DynamicArray<Tomo> buscarPorNombre(String nombre){
		return null;
		
	}

	public DynamicArray<Tomo> buscarPorTipo(Categoria categoria){
		return null;
	...
	public void actualizarComic(Comic comic, int tipoOperacion, String nuevaInformacion){
        ...
        private Tomo buscarTomo(String tomo){
        	...
        	{
        		if(tomo.equalsIgnoreCase(this.getTomos().get(i).getNombre())) return this.getTomos().get(i);
        	}
        	return null;
        }
